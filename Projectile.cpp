...
double getDrag( double v, double m) {
    ...
    double Fd = environment.AIR_DENSITY*pow(v, 2)*A* drag_coeff/2.0;
    return Fd;
} //end getDrag
...
double fy(double tk, double vxak, double vyk){
    ...
    double fy = -1.0*environment.G - Fd*vyk/(m*v);
    return fy;
} //end fy
...
std::vector<double> Projectile::updatePosition(double current_time, double DELTA_T, Environment env) {
    ...
    std::vector<double> temp = RungeKutta.rungeKutta(this, DELTA_T, current_time - getCreationTime());
    velocity[0] += temp[0];
    velocity[1] += temp[1];
